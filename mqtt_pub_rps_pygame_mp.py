...
def on_connect(client, userdata, flags, rc):
  ...
  client.subscribe("ece180d/testk")
        ...
        if event.type == KEYDOWN:
            if event.key in [pygame.QUIT, pygame.K_ESCAPE]:
                ...
                if event.key == pygame.K_UP:
                    ...
                    client.publish("ece180d/sub", "rock", qos=1)
                if event.key == pygame.K_LEFT:
                    # User chose paper
                    pubPygameInput = "paper"
                    own_data_received = True
                    client.publish("ece180d/sub", "paper", qos=1)
                if event.key == pygame.K_RIGHT:
                    # User chose scissors
                    pubPygameInput = "scissors"
                    own_data_received = True
                    client.publish("ece180d/sub", "scissors", qos=1)
            #if own_data_received:
                #print(pubPygameInput)

    if own_data_received and opp_data_received:
        temp = determineWinner(pubPygameInput,subPygameInput)
        if (temp == 0):
            GRAY = (150,150,150)
            screen.fill(GRAY)
